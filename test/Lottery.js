...
contract("Lottery", accounts => {
    ...
    it("requires a minimum amount of ether to enter", async () => {
        const lottery = await Lottery.deployed();
        try {
            await lottery.enter({from: web3.eth.accounts[1], value: 0});
            assert.fail();
        }
        catch (err)
        {
    ...
    it("should allow one manager to pick a winner", async () => {
        const lottery = await Lottery.deployed();
        await lottery.enter({from: web3.eth.accounts[1], value: web3.toWei(0.1, "ether")});
        try {
            await lottery.pickWinner({from: web3.eth.accounts[1]});
            assert.fail();
        }
        catch (err)
        {
    ...
    it("should send money to the winner", async () => {
        ...
        assert.ok(0 == web3.eth.getBalance(lottery.address));
    });

    it("should allow multiple accounts to enter", async () => {
        const lottery = await Lottery.deployed();
        await lottery.enter({from: web3.eth.accounts[1], value: web3.toWei(0.1, "ether")});
        await lottery.enter({from: web3.eth.accounts[2], value: web3.toWei(0.1, "ether")});
        await lottery.enter({from: web3.eth.accounts[3], value: web3.toWei(0.1, "ether")});
        await lottery.enter({from: web3.eth.accounts[4], value: web3.toWei(0.1, "ether")});
        await lottery.enter({from: web3.eth.accounts[5], value: web3.toWei(0.1, "ether")});
